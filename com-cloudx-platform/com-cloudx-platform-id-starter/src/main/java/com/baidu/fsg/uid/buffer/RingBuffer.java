...
/**
 * Represents a ring buffer based on array.<br> Using array could improve read element performance due to the CUP cache line. To prevent the side effect of False Sharing, {@link PaddedAtomicLong} is
 * using on 'tail' and 'cursor'<p>
 * <p>
...
public class RingBuffer {
     ...
     *
     * @param bufferSize    must be positive & a power of 2
     * @param paddingFactor percent in (0 - 100). When the count of rest available UIDs reach the threshold, it will trigger padding buffer<br> Sample: paddingFactor=20, bufferSize=1000 ->
    ...
    /**
     * Put an UID in the ring & tail moved<br> We use 'synchronized' to guarantee the UID fill in slot & publish new tail sequence as atomic operations<br>
     *
     ...
     * <p>
     * Before getting the UID, we also check whether reach the padding threshold, the padding buffer operation will be triggered in another thread<br> If there is no more available UID to be taken,
     * the specified {@link RejectedTakeBufferHandler} will be applied<br>
     *
     * @return UID
    ...
    public String toString() {
        ...
        builder.append("RingBuffer [bufferSize=").append(bufferSize).append(", tail=").append(tail).append(", cursor=").append(cursor).append(", paddingThreshold=").append(paddingThreshold)
                .append("]");
