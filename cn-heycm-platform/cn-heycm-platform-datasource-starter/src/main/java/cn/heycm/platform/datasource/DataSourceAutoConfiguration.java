...
public class DataSourceAutoConfiguration {
    ...
    public DataSourceAutoConfiguration(NacosConfListener nacosConfListener, DataSourceProperties properties) {
        ...
        nacosConfListener.addListener(DATA_ID, (config) -> {
            ...
            Optional<DataSourceItem> find = list.stream().filter(item -> properties.getSchema().equals(item.getSchema())).findAny();
            Assert.isTrue(find.isPresent(), "Can not find datasource config by schema: " + properties.getSchema());
            dataSourceItem = find.get();
        });
